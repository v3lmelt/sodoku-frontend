...
export default {
  ...
  methods: {
    highlightCells(cellIndex) {
      let cells = document.querySelectorAll('.test-cells');
      cells.forEach((cells) => {
        cells.classList.remove('test-highlight');
        cells.classList.remove('test-highlight2');
      });
      // 将 NodeList 转换为数组
      ...
      targetCells.forEach((cells) => {
        cells.classList.add('test-highlight');
      });

      targetCell.classList.add('test-highlight2');
    }
  }
};
</script>
<template>
  <div>
    <div class="test-help-button">帮助</div>
    <div class="test-large-square">
      <div v-for="(smallSquares, index) in smallSquares" :key="index" class="test-small-square">
        <span
